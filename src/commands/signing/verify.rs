...
impl Verify {
    fn cmd_verify(
        publickey_path: PathBuf,
        pbo_path: PathBuf,
        signature_path: Option<PathBuf>,
    ) -> Result<(), ArmakeError> {
        let publickey =
            BIPublicKey::read(&mut File::open(&publickey_path).expect("Failed to open public key"))
...
impl Command for Verify {
    fn register(&self) -> clap::App {
        clap::SubCommand::with_name("verify")
            ...
            .arg(clap::Arg::with_name("signature").help("Signature file (.bisign)"))
    }
